...
from utils import model_utils, public_utils, fed_avg_top_k, log_util, SPFSManager
...
def train(seed=50, client_num=16, batch_size=100, epoch=200, local_sgd=10, lr=0.1
    ...
    csv_name = os.path.join(csv_dir,
                            f'{os.path.basename(__file__)[:-3]} spares={sparse} lr={lr} n={client_num} check={check} seed={seed}')
